...
#'
#' For more information about Fop \href{https://pubmed.ncbi.nlm.nih.gov/6175758/}{Ikemura 1981}.
#'
#' @usage FOP.values <- function(df.fasta, genetic.code = "1", threshold = 0, alt.init = TRUE , stop.rm = FALSE, filtering = "none", host = "none")
#'
...
#' @param host Fop could be calculated without refrence set host = "none". If host = to a list of dataframe of host genes returned from read.host function it will be calculated using the host genes as a refrence set. 
#'
#' @return A list of data.frame(s) containing the computed Fop values for each DNA sequences.
#'
#' @import coRdon
...
#' Fop <- FOP.values(fasta.virus, host = fasta.host)
#' }
#' @export
#'
...
FOP.values <- function(df.fasta, genetic.code = "1", 
  ...
  for(i.fasta in 1:length(df.fasta)){
    ...
    tryCatch({
      fop.list[dna@virus.name] <- list(fop)},
      error = function(e){
        fop.list[dna@host.name] <<- list(fop)}
    )
